...
export async function saveTeamEdit(args: {
  ...
  // Snapshot: update existing draft if there is one for this season, otherwise
  // insert. (Strategy: one editable snapshot per team+season; history comes
  // from the audit_log diffs.)
  ...
  // Bump the team's own updated_at + status to match.
  await supabase
    .from('teams')
    .update({ status: args.status, last_edited_by: user.id })
    .eq('id', args.teamId);
